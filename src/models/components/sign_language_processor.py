...
class SignLanguageProcessor(ProcessorMixin):
    r"""
    ...
    def __call__(self, *args, **kwargs):
        ...
        if video_frames is None and text_tokens is None:
            raise ValueError("You need to specify either an `video_frames` or `text_tokens` input to process.")

        if audio is not None:
            inputs = self.feature_extractor(audio, *args, sampling_rate=sampling_rate, **kwargs)
        if text_tokens is not None:
            encodings = self.tokenizer(text_tokens, **kwargs)
