...
def process_segment(video_path: str, subtitle_item: SubRipItem, frame_rate: float, frame_width: int, frame_height: int,
                    frame_size_multiplier: float, face_detection, output_folder: str, idx: int):
    ...
    start_time -= pre_start_seconds
    end_time += post_end_seconds
    ...
    while cap.isOpened() and i <= end_frame:
        ...
        if i % frame_rate_divisor == 0:
            ...
            if signer_frame is None or signer_frame.size == 0:
                logging.error(f"Signer frame is empty at frame {i}. Skipping.")
                continue

            try:
                signer_frame_rgb = cv2.cvtColor(signer_frame, cv2.COLOR_BGR2RGB)
                results = face_detection.process(signer_frame_rgb)
                ...
                logging.error(f"Failed to update bounding box at frame {i}: {e}")

            if bounding_box_buffer:
...
def process_video(video_path: str, subtitle_path: str, output_folder: str):
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for idx, sub in enumerate(subs_items):
            ...
            futures.append(executor.submit(process_segment, video_path, sub, frame_rate, frame_width, frame_height,
                                           frame_size_multiplier, face_detector, output_folder, idx))

        for future in futures:
            # Wait for future. We need to keep the order of the subtitles!
